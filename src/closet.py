...
def plot_variable_distributions(X_train, continuous_vars, discrete_vars, categorical_vars, binary_vars, binary_labels):
    # Get the Seaborn color palette
    palette = sns.color_palette("RdPu", n_colors=10).as_hex()

    # Function to plot continuous variables
    def plot_continuous_vars(df, vars):
        for var in vars:
            fig = make_subplots(rows=1, cols=2, subplot_titles=(f'Histogram of {var}', f'Boxplot of {var}'))
            
            # Histogram with edges
            hist = go.Histogram(x=df[var], nbinsx=30, marker=dict(color=palette[5], line=dict(color='black', width=1)))
            fig.add_trace(hist, row=1, col=1)
            
            # Boxplot
            box = go.Box(y=df[var], marker_color=palette[3])
            fig.add_trace(box, row=1, col=2)
            
            fig.update_layout(title_text=f'Distribution of {var}', showlegend=False)
            fig.show()

    # Function to plot discrete variables
    def plot_discrete_vars(df, vars):
        for var in vars:
            fig = go.Figure(data=[go.Histogram(x=df[var], nbinsx=30, marker=dict(color=palette[6], line=dict(color='black', width=1)))])
            fig.update_layout(title=f'Histogram of {var}', xaxis_title=var, yaxis_title='Frequency')
            fig.show()

    # Function to plot categorical variables
    def plot_categorical_vars(df, vars):
        for var in vars:
            fig = go.Figure(data=[go.Histogram(y=df[var], marker_color=palette[9], marker_line=dict(color='black', width=1))])
            fig.update_layout(title=f'Bar Plot of {var}', xaxis_title='Count', yaxis_title=var)
            fig.show()

    # Function to plot binary variables
    def plot_binary_vars(df, vars, labels):
        for var in vars:
            fig = go.Figure(data=[go.Histogram(x=df[var], marker_color=palette[3], marker_line=dict(color='black', width=1))])
            label_0, label_1 = labels.get(var, ('0', '1'))
            fig.update_layout(title=f'Bar Plot of {var} ({label_0} vs {label_1})', xaxis_title=var, yaxis_title='Count')
            fig.show()

    # Plot continuous variables
    plot_continuous_vars(X_train, continuous_vars)

    # Plot discrete variables
    plot_discrete_vars(X_train, discrete_vars)

    # Plot categorical variables
    plot_categorical_vars(X_train, categorical_vars)

    # Plot binary variables
    plot_binary_vars(X_train, binary_vars, binary_labels)

def plot_target_distribution(y_train):
    # Get the Seaborn color palette
    palette = sns.color_palette("RdPu", n_colors=10).as_hex()
    
    # Plot the target variable (Diagnosis)
    fig = go.Figure(data=[go.Histogram(x=y_train, marker_color=palette[0], marker_line=dict(color='black', width=1))])
    fig.update_layout(title='Bar Plot of Diagnosis (0 vs 1)', xaxis_title='Diagnosis', yaxis_title='Count')
    fig.show()
